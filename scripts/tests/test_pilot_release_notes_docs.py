...
class PilotReleaseNotesDocsTests(unittest.TestCase):
    def test_release_notes_name_runnable_local_product_and_gate(self) -> None:
        ...
        self.assertIn("Baseline product/runtime commit checked: `1929dc0`", text)
        self.assertNotIn("Baseline commit checked: `04fb56b`", text)
        self.assertIn("actions/runs/25663480672", text)
        self.assertIn("6f005aaa6c343f62558cc6ee45034327fcf39464", text)
        self.assertIn("actions/runs/25672743585", text)
        self.assertIn("Add batch guard to send-copy metadata", text)
        self.assertIn("cd prophet-console && npm run acceptance", text)
        self.assertIn("make pilot-ready-check-full DATE=2026-05-11", text)
        self.assertIn("Playwright console smoke tests", text)
        self.assertIn("389 tests", text)
        ...
        self.assertIn("true GitHub fresh clone of GitHub `main` at verification time", text)
        self.assertIn("91b91e630a418b83cb9c00ff959e47be5897ac6a", text)
        self.assertIn("checked-baseline true fresh-clone proof", text)
        self.assertIn("ad5eb5c", text)
        self.assertIn("documentation-only commits", text)
        self.assertIn("should not be appended here one-by-one", text)
        self.assertIn("make validation-send-copy-check DATE=2026-05-11", text)
        self.assertIn("without changing the product/runtime baseline", text)
        self.assertIn("no policy drift", text)
    ...
    def test_linux_fresh_clone_smoke_is_ci_backed(self) -> None:
        ...
        self.assertIn("being forced to run on Node 24", notes)
        self.assertIn("Documentation-only commits", notes)

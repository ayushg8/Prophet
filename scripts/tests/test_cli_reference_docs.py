...
class CliReferenceDocsTests(unittest.TestCase):
    def test_readme_documents_validation_status_machine_readable_output(self) -> None:
        ...
        self.assertIn("make validation-working-product-handoff-save", text)
        ...
        self.assertIn("validation/private/contact-form-copy-YYYY-MM-DD/", text)
    ...
    def test_validation_status_documents_machine_readable_output(self) -> None:
        ...
        self.assertIn("fail closed", text)
        self.assertIn("dry-run pre-send wrapper", text)
        self.assertIn("one dry-run", text)
        self.assertIn("python3 scripts/validation-pre-send-check-all.py --help", text)
        self.assertIn("--refresh-readme", text)

...
class FinishInventoryDocsTests(unittest.TestCase):
    ...
    def test_completion_audit_names_latest_implementation_checkpoint(self) -> None:
        ...
        self.assertIn("6f005aaa6c343f62558cc6ee45034327fcf39464", audit)
        self.assertIn("actions/runs/25672743585", audit)
        self.assertIn("Add batch guard to send-copy metadata", audit)
        self.assertIn("Audit-only handoff commits after that implementation checkpoint", audit)
        self.assertIn("intentionally not hardcoded", audit)
        ...
        self.assertIn("gh run list --repo Ayush1298567/Prophet --branch main --limit 1", audit)
    ...
    def test_finish_inventory_names_latest_implementation_checkpoint(self) -> None:
        ...
        self.assertIn("6f005aaa6c343f62558cc6ee45034327fcf39464", inventory)
        self.assertIn("actions/runs/25672743585", inventory)
        self.assertIn("Add batch guard to send-copy metadata", inventory)
        self.assertIn("Audit-only handoff commits after that implementation checkpoint", inventory)
        self.assertIn("intentionally not hardcoded", inventory)

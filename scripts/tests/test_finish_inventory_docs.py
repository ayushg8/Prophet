...
class FinishInventoryDocsTests(unittest.TestCase):
    ...
    def test_release_hygiene_checks_are_recorded(self) -> None:
        ...
        for text in (audit, inventory):
            with self.subTest(document=("audit" if text == audit else "inventory")):
                ...
                self.assertIn("91 Markdown", text)
    ...
    def test_completion_audit_names_latest_implementation_checkpoint(self) -> None:
        ...
        self.assertIn("6e7ac8592dbcba868e9cbab6389ed2cb07c13837", audit)
        self.assertIn("actions/runs/25688231405", audit)
        self.assertIn("Add working product handoff generator", audit)
        self.assertIn("current verified `main` head", audit)
        self.assertIn("eaafaacd8d27edca35cf89491a7116a4a848471b", audit)
        self.assertIn("actions/runs/25690746584", audit)
        self.assertIn("Document send batch readiness handoff", audit)
        self.assertIn("make validation-working-product-handoff-save DATE=2026-05-11", audit)
        self.assertIn(
        ...
        self.assertIn("python3 -m unittest discover -s scripts/tests -v", audit)
        self.assertIn("make release-hygiene", audit)
        self.assertIn("PROPHET_CONTROL_PORT=8891 PROPHET_CONSOLE_PORT=5291 make console-live-check", audit)
        ...
        self.assertIn("workflow commits after `eaafaac`", audit)
    ...
    def test_finish_inventory_names_latest_implementation_checkpoint(self) -> None:
        ...
        self.assertIn("6e7ac8592dbcba868e9cbab6389ed2cb07c13837", inventory)
        self.assertIn("actions/runs/25688231405", inventory)
        self.assertIn("Add working product handoff generator", inventory)
        self.assertIn("Current verified `main` head", inventory)
        self.assertIn("eaafaacd8d27edca35cf89491a7116a4a848471b", inventory)
        self.assertIn("actions/runs/25690746584", inventory)
        self.assertIn("Document send batch readiness handoff", inventory)
        self.assertIn("make validation-working-product-handoff-save", inventory)
        self.assertIn("TARGET=target-dib-platform-001 DATE=2026-05-11", inventory)
        self.assertIn("python3 -m unittest discover -s scripts/tests -v", inventory)
        self.assertIn("make release-hygiene", inventory)
        self.assertIn("make pilot-ready-check-full DATE=2026-05-11", inventory)
        self.assertIn("PROPHET_CONTROL_PORT=8891 PROPHET_CONSOLE_PORT=5291 make", inventory)
        ...
        self.assertIn("Product or workflow commits after `eaafaac`", inventory)

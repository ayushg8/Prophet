...
class SoftwareSupplyChainPacketDocsTests(unittest.TestCase):
    def test_packet_contains_required_review_sections(self) -> None:
        ...
        self.assertIn("make supply-chain-sbom-check DATE=YYYY-MM-DD", text)
        ...
        self.assertIn("source-file SHA-256", text)
        self.assertIn("component counts", text)
        self.assertIn("review-boundary non-claims", text)

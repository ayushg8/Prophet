    ...
    if outreach.get("available"):
        ...
        lines.extend(
            [
                ...
                f"- Send-copy batch do-not-send guard: {outreach.get('send_copy_batch_do_not_send_path') or 'none'}",
...
def _outreach_execution(
    *,
    ...
    return {
        ...
        "send_copy_batch_do_not_send_exists": send_copy_batch_check["do_not_send_exists"],
...
def _send_copy_batch_check(
    *,
    ...
    do_not_send_path = send_copy_batch_dir / "DO_NOT_SEND.md"
    do_not_send_exists = do_not_send_path.exists()
    ...
    result = {
        ...
        "do_not_send_exists": do_not_send_exists,
        "matches": False,
        "state": "missing",
    ...
    if not subject_order_exists:
        ...
        return result
    if not do_not_send_exists:
        result["state"] = "stale"
        result["mismatch_reason"] = "send-copy batch DO_NOT_SEND guard is missing"
        return result
    try:
    ...
    try:
        ...
        actual_do_not_send = do_not_send_path.read_text(encoding="utf-8")
    ...
    if actual_subject_order != expected_subject_order:
        ...
        return result
    expected_do_not_send = _expected_send_copy_batch_do_not_send(
        generated_for=generated_for,
        copy_file_count=len(expected_files),
    )
    if actual_do_not_send != expected_do_not_send:
        result["state"] = "stale"
        result["mismatch_reason"] = "send-copy batch DO_NOT_SEND guard does not match current generator"
        return result
        ...
        "files": [
            {
                ...
                "pre_send_check_command": file["pre_send_check_command"],
...
def _expected_send_copy_batch_operator_notes(generated_for: str) -> list[str]:
    return [
        ...
        f"Before a full-block send, run make validation-pre-send-check-all DATE={generated_for}.",
...
def _expected_send_copy_batch_readme(*, generated_for: str, copy_file_count: int) -> str:
    return "\n".join(
        [
            ...
            f"- Before sending the full block, run `make validation-pre-send-check-all DATE={generated_for}`.",
...
def _expected_send_copy_batch_checklist(
    *,
    ...
    lines = [
        ...
        f"Before a full-block send, also run `make validation-pre-send-check-all DATE={generated_for}` and proceed only if it passes.",
        "",
        "For each row:",
        "",
...
def _expected_send_copy_batch_do_not_send(
    *,
    generated_for: str,
    copy_file_count: int,
) -> str:
    return "\n".join(
        [
            ...
            "only after `make validation-send-copy-check` and",
            "`make validation-pre-send-check-all` pass for this date.",
            "",

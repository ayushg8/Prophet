...
`manifest.json`, `CHECKLIST.md`, `COPY_ONLY_INDEX.md`, `SUBJECT_ORDER.md`,
`DO_NOT_SEND.md`, and batch `README.md` are private tracker/operator metadata
and should not be pasted or sent to a buyer; do not send the private manifest,
checklist, copy index, subject-order helper, DO_NOT_SEND guard, or batch README.
The dashboard verifies the directory through
...
tracker/log state. It also names send-copy and contact-form copy readiness
checks. The handoff states that recipient names, emails, LinkedIn URLs,
outbound channel details, the external outreach channel, and the real contact must come from outside the repo before a draft can actually be sent.

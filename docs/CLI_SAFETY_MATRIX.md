...
| `python3 scripts/validation-sprint-dashboard.py` | Combines validation scorecards, keeps `pilot_pull_detected` separate from `build_next_slice`, requires `target_backed_validation` to reach `build_next_slice` before opening production scope, counts only interviews tied to anonymized targets in `call_booked` or `completed` with matching segment/persona metadata for that gate, supports `--require-date` for date-mismatched private message packs, reports `next_draft_state`, `send_copy_state`, `send_copy_batch_state`, `send_copy_batch_readme_exists`, `send_copy_batch_checklist_exists`, `send_copy_batch_copy_index_exists`, `send_copy_batch_subject_order_exists`, `send_copy_batch_do_not_send_exists`, `contact_form_copy_state`, `contact_form_copy_matches_current_pack`, and `next_pending_pre_send_check_command`, verifies the existing next draft's target/date/status/body through `next_draft_matches_next_pending`, verifies the copy-only send text through `send_copy_matches_next_pending`, verifies whole-block copy files, manifest fields, manifest operator notes, manifest outbound-boundary fields, copy-file SHA-256 values, batch README body, batch checklist body with per-draft pre-send commands, neutral copy-index body, subject-order body, and DO_NOT_SEND guard through `send_copy_batch_matches_current_pack` before treating the batch path as usable, verifies the compact contact-form copy directory against the current verified drafts before reporting `contact_form_copy_matches_current_pack`, points operators to the one-draft outreach path plus exact Make dry-run / pre-send / `CONFIRM_SENT=1` commands when a private message pack exists, and supports `--format team` for sanitized aggregate-only shared updates that omit target labels, commands, message bodies, and private validation paths while showing gate-counted buyer evidence before any raw example-seed counts. |
| `python3 scripts/validation-next-action.py` | Renders a private ignored `NEXT_ACTION.md` handoff from the current dashboard; `--date` rejects stale packs, `--out` writes the ignored handoff, records the local git head/worktree state, best-effort GitHub CI status, the single-target pre-send command, the full-batch pre-send command, send-copy readiness checks, and contact-form copy readiness checks for restored-session context; states that recipient names, emails, LinkedIn URLs, outbound channel details, the external outreach channel, and the real contact live outside the repo; and the helper does not send outreach, prune artifacts, confirm writes, or mutate trackers/logs. |
| `python3 scripts/check-buyer-follow-up-package.py` | Verifies the qualified-buyer follow-up package after a smoke run: tracked buyer docs exist, required buyer-boundary phrases remain present, generated evidence/handoff artifacts exist under ignored runtime paths, runtime files stay untracked, hashes match `scripts/pilot-demo-smoke.sha256`, evidence safety/redaction attestations are present, the handoff manifest is `review_template_only`, customer placeholder mapping is required, and the review ZIP is valid. It does not send outreach, open the build gate, or treat runtime artifacts as release-bound commit content. The `make buyer-follow-up-check` wrapper first refreshes the fixture smoke, which writes ignored runtime outputs before running this verifier, so console-demo runtime artifacts cannot stale the package check. |
| `python3 scripts/validation-weekly-review.py` | Builds a read-only private weekly review, validates the private tracker/log, reports message-pack age, date-guarded outreach execution readiness, send-copy batch README/checklist/copy-index/subject-order and DO_NOT_SEND guard state, contact-form copy state when present, ignored private artifact counts, stale private artifacts, unsafe/outdated send-copy and contact-form copy warnings, and pruning candidates, and does not delete files or mutate trackers/logs. |
| `python3 scripts/validation-prune-private.py` | Builds a dry-run pruning plan from the weekly review for generated ignored private validation artifacts only, including outdated send-copy and contact-form copy batch directories, protects validation trackers/logs/templates/README files, requires matching `--review-date`, and removes eligible artifacts only with `--confirm-prune`; the Make wrapper writes only with `CONFIRM_PRUNE=1`. |
...
`send_copy_batch_do_not_send_exists`, are true. Do not attach the files; the
...
that handoff; it does not send, delete, or mutate trackers/logs. It also
reminds operators that the external outreach channel and real contact must be
